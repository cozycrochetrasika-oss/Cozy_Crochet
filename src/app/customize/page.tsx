...
export default function CustomizePage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Also persist custom request to server-side repository
    fetch('/api/custom-requests', {
      ...
      body: JSON.stringify({
        ...
        colorPreferences: formData.colors,
        description: `Size: ${formData.size} | Qty: ${formData.quantity} | Target Date: ${formData.desiredDate || 'Flexible'} | Contact Method: ${formData.contactMethod}\n\nDetails: ${formData.description}`,
        budgetPaise: formData.budgetRange.includes('1,500') ? 150000 : 250000,
        referenceImages: [],
      }),
    }).catch((err) => console.error('[Customize] Error saving custom request to server:', err));
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-16">
      ...
      {submitted ? (
        <div className="p-8 sm:p-12 rounded-3xl bg-white border border-pink-100 shadow-md text-center space-y-8">
          <div className="w-16 h-16 rounded-full bg-pink-50 text-pink-600 flex items-center justify-center mx-auto shadow-xs">
          ...
          <div className="p-6 rounded-2xl bg-pink-50/40 border border-pink-100 text-left max-w-lg mx-auto space-y-2 text-xs text-text-secondary">
            <div className="font-bold text-ink text-sm pb-1 border-b border-pink-100">Order Brief Summary</div>
          ...
          <div className="space-y-3 pt-2 max-w-md mx-auto">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="w-full py-3.5 px-4 rounded-xl bg-pink-600 hover:bg-pink-700 text-white font-semibold text-sm shadow-sm transition-all flex items-center justify-center gap-2.5"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="w-full py-3 px-4 rounded-xl bg-pink-50 hover:bg-pink-100 text-pink-700 border border-pink-200 font-semibold text-xs shadow-xs transition-colors flex items-center justify-center gap-2"
            >
              <MessageCircle className="w-4 h-4" />
        ...
        <form onSubmit={handleSubmit} className="p-6 sm:p-10 rounded-3xl bg-white border border-pink-100 shadow-sm space-y-6">
          {/* Customer Info */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 rounded-xl border border-pink-200 bg-white text-ink outline-none focus:border-pink-400 focus:ring-1 focus:ring-pink-400 text-sm"
              />
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="e.g. Baby Pink, Blush, Dusty Rose & Ivory"
                className="w-full px-4 py-2.5 rounded-xl border border-pink-200 bg-white text-ink outline-none focus:border-pink-400 focus:ring-1 focus:ring-pink-400 text-sm"
              />
            </div>
          ...
          <div>
            ...
            <div className="border-2 border-dashed border-pink-200 hover:border-pink-400 rounded-2xl p-4 text-center bg-pink-50/20 cursor-pointer transition-colors">
          ...
          <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <button
              type="submit"
              className="w-full sm:w-auto px-8 py-3.5 rounded-xl bg-pink-600 hover:bg-pink-700 text-white font-semibold text-sm shadow-sm flex items-center justify-center gap-2 transition-all"
            >
